...
@Component
public class SubscribeToNeoCommandHandler implements UpdateHandler{

    private final TelegramClient telegramClient;
    private final UserRepository userRepository;

    @Autowired
    public SubscribeToNeoCommandHandler(TelegramClient telegramClient, UserRepository userRepository) {
    ...
    public boolean supports(Update update) {
        return update != null && update.hasMessage() && update.getMessage().hasText()
                && update.getMessage().getText().trim().equalsIgnoreCase("/subscribe_to_neo");
    }
    ...
    public void handle(Update update) {
        ...
        user.setSubscribedToNeo(true);
        userRepository.save(user);

        try {
            telegramClient.execute(SendMessage.builder()
                    .chatId(update.getMessage().getChatId())
                    .text("You have successfully subscribed to the daily NEO!")
                    .build());
